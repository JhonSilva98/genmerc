...
class _MyVenderState extends State<MyVender> {
  ...
  List<String> listaIDContain = [];

  double subtotal = 0.0;
  double troco = 0;
  double quantidade = 0;

  final TextEditingController _valorpago = TextEditingController(text: '0.0');
  ...
  Future<void> funcionAttVendas() async {
    ...
    for (String i in listId) {
      if (!listaIDContain.contains(i)) {
        listaIDContain.add(i);
        ...
        subtotal += double.parse((subto).toStringAsFixed(2));
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: StreamBuilder(
          stream: Firestore.instance.collection('venda').stream,
